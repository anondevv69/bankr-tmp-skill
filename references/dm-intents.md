...
**Path K — Claim fees on hybrid unit (ERC-1155):** Resolve **linked wallet** → **`GET /api/claim/hybrid-status?token=0x…&wallet=<linked>`** → read **`proof.claimPaysAllHolders`** → **`claimFeesForToken`** (pays **all** holders). Read **`hybrid-claim-fees.md`**. **Forbidden:** ask user for wallet; Bankr **`collectFees`** from user wallet.

**Path L — Tweet / @bankr hybrid claim:** Same as **Path K**. User tweet needs only launch token **`0x…`** (optional **`serial=N`**). Bankr uses linked wallet — **never** ask user to paste wallet or contract if already in tweet.
