...
**Path K — Claim fees on hybrid unit (ERC-1155):** **`GET /api/claim/hybrid-status?token=0x…&wallet=0x…`** → **`claimFeesForToken`**. Read **`hybrid-claim-fees.md`**. **Forbidden:** Bankr **`collectFees`** from user wallet.

**Path L — Tweet / @bankr hybrid claim:** Same as **Path K** when tweet includes launch token **`0x…`** or user’s X is linked to Bankr wallet. **Do not** ask for contract address if already in tweet. **Do not** hand off to website only when prepare/confirm is available. Ticker-only (`$CTO`) → API resolve or one clarifying question.
