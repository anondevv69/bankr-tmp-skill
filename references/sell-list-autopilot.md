...
4. **`POST /api/list/dual`** + execute `site.steps[]` + **OpenSea** (default dual — `LISTING-VENUES.md`). Skip OpenSea if `password` or site-only.
...
8. **OpenSea (default dual):** after site live — opensea skills; skip if password / max-per-wallet (`LISTING-VENUES.md`).
