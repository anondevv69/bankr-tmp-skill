...
7. **Reply (mandatory on X/tweet):** plain English + **full** `https://www.tokenmarketplace.shop/listing/sale/{listingId}` from **`siteListingUrl`** in status response (or build from `siteListing.listingId`).
